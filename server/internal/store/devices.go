...
const deviceSelect = `
	SELECT d.id, d.site_id, s.customer_id, s.name, c.name,
	       d.hostname, d.os, d.arch, d.agent_version, d.status, d.tags, d.last_seen_at, d.created_at
	FROM devices d
...
func scanDevice(row pgx.Row) (Device, error) {
	...
	err := row.Scan(&d.ID, &d.SiteID, &d.CustomerID, &d.SiteName, &d.CustomerName,
		&d.Hostname, &d.OS, &d.Arch, &d.AgentVersion, &d.Status, &d.Tags, &d.LastSeenAt, &d.CreatedAt)
